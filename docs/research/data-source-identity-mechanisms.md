...
| **PostgreSQL** (via Ontop) | The `cdf_ro` role above with `statement_timeout` and a connection limit; Ontop's `_FILE` secrets | Either per-user Postgres roles with RLS keyed off `current_user` and the fabric issuing `SET ROLE` per leg (requires a leg-scoped connection, not a shared Ontop pool), or PG18 `oauth` with a validator module. Note Ontop itself has no delegation hook, so this needs a native Postgres executor or an Ontop change |
