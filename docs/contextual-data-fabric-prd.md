...
>
> **v0.3 (2026-07-14):** absorbed the deep-analysis passes. **ADR-0001** (M5) decides the conceptual-query IR — typed graph-pattern serializing to **SPARQL**, `CSI v1` as the mapping hub, Ontop buy-vs-build open (§9.10) — making M5 mostly integration of owned components (`arango-sparql-py`, `arango-cypher-py`, the analyzers) *(corrected 2026-09-15: Ontop adopted — ADR-0001 #2 and its 2026-09-10 amendment; `arango-cypher-py` never became a CDF dependency, see §5)*. AOE PRD **§6.17–§6.19** definitizes alignment / A-box / competency questions; r2g Phase 12 is reframed around CSI+R2RML. **Use cases formalized** from PJ's 12 locked questions (`docs/use-cases.md`, §4); `customer-context` cloned + verified.
>
...
- Analogue: the Cypher/SPARQL **transpiler** (English → Cypher → AQL), but general-purpose and federated — it must *break the query up*, run the parts, and stitch results. **Now decided (ADR-0001, M5):** the conceptual-query IR is a **typed graph-pattern IR serializing to SPARQL** (the OBDA/VKG pattern); the owned transpiler `arango-sparql-py` (SPARQL→AQL) carries the Arango leg and **Ontop** the SQL leg (adopted, ADR-0001 #2; the Cypher→AQL fallback through `arango-cypher-py` was withdrawn and CDF neither installs nor imports that package — its NL-engine design was reimplemented in `src/cdf/query/nl.py` on `arango-query-core`; corrected 2026-09-15) — M5 is mostly **integration of owned components**; the net-new build is the federation layer (partition planner, canonical-key join, provenance).
...
- **★ RESOLVED (v0.2): does structured→ontology exist? Yes — twice over, with documented ownership.** AOE's capability table marks **Schema → Ontology (structured): Done** (ArangoDB collection schemas and relational/SQL schemas → OWL/SHACL classes/properties/constraints), with the split defined as *"AOE owns the SQL→OWL/SHACL mapping; `relational-schema-analyzer` is a read-only physical-schema introspector."* Independently, **RSA** (PyPI; v0.4.0 when this was written, **0.8.0** as of 2026-09-15 with bitemporal stamping) emits `{conceptualSchema, physicalMapping, metadata}` bundles + OWL export from 7 live sources (incl. Postgres, Snowflake, Databricks) and dbt/OSI catalogs. r2g Phase 10 (implemented) adds LLM-assisted ontology derivation with a human review UI — the "confirm ~2%" step of B2 already has a working surface. **B1 is wiring, not building.**
  - **★ CORRECTED (2026-09-14, code-read):** the structured path CDF *actually* uses is **r2g → RSA → CSI + R2RML**, not AOE. Every relational CSI in `deploy/csi/` carries `provenance.producer: r2g`; r2g derives the conceptual model through RSA (`r2g/rsa_ontology.py`, RSA a core dependency) and emits CSI/R2RML; the Arango leg's CSI is produced by ASA directly (`deploy/arango/export_csi.py`). CDF imports no AOE, ASA or RSA code at runtime. AOE's SQL→OWL/SHACL path (`relational_schema_extraction.py`, RSA behind an optional extra) is a **second, independent** structured→ontology derivation that CDF never calls, so the two can drift with nothing to catch it. The v0.2 "AOE owns" split above stands as AOE's own scope; it does not describe CDF's pipeline. Evidence and the plan to reconcile the two derivations: `docs/research/unified-ontology-mapping-architecture.md` (breaks 2–4, Q-1, Q-5, §9 steps 3–4).
...
- **`arango-sparql-py`** (`~/code/arango-sparql-py`; primary `arango-solutions`, mirror `ArthurKeen`; **not on PyPI** — CDF installs it by git SHA from `deploy/pins/arango-sparql-py.txt` under CC-9, the analyzer band riding along) — **SPARQL 1.1 → AQL transpiler** (rdflib + OWL/Turtle schema ontology; broad translation coverage, injection-safe, ~781 tests). Per ADR-0001: the **owned Arango leg** for the SPARQL IR. **The finishing work is DONE (2026-07-15):** WPs A2/A3/C1/C2 landed — evaluation correctness CI-gated (live-Arango + W3C suites), CSI→MappingBundle adapter, and the `translate_partition` federation entry (canonical keys, `seed_bindings` bind-join pushdown, `as_of`; contract: `arango-sparql-py/docs/architecture/proposals/federation-entry-point.md`).
- **`arango-cypher-py`** (`~/code/arango-cypher-py`, v0.2.0, ArthurKeen) — **openCypher → AQL transpiler** (TCK core ~90%) plus the stack's best asset: a **proven, IR-agnostic NL→conceptual-query engine** (few-shot, fuzzy entity resolution, EXPLAIN-grounded self-healing, eval harness at 93–100%). Per ADR-0001: reused to *generate* the SPARQL IR (seam swap, WP D1) and available as the P1 Arango-leg fallback; its eval harness seeds M10. *(Corrected 2026-09-15: the fallback was withdrawn and CDF has never declared, installed or imported `arango-cypher-py`; the NL-engine design landed in `src/cdf/query/nl.py`, the eval-harness design in `src/cdf/eval/`. Not on PyPI; its analyzer band is kept aligned with `arango-sparql-py`'s.)*
...
2. ~~**Structured→ontology readiness (★)**~~ — **RESOLVED (v0.2):** confirmed done in both AOE (owns SQL→OWL/SHACL) and RSA (introspection + tool-contract bundle). Remaining sub-decision: which path B1 demos with (proposed: RSA introspection bundle → AOE OWL/SHACL mapping, matching the documented split). → §7.6 **Settled by what shipped (2026-09-15):** B1 runs **r2g → RSA → CSI v1 + R2RML** (every relational CSI under `deploy/csi/` is r2g-emitted); AOE is not on CDF's structured path (§7.6 correction), and AOE's structured entry point is now CSI import, not an RSA-bundle handoff.
...
- **Identity model:** the fabric connects **as itself** by default — one least-privilege, **read-only** service identity per source (`service` trust level). This is the certified baseline, not a demo limitation: every graph-virtualization and SQL-federation product surveyed ships it as the default ([vendor survey](research/vendor-auth-access-control-survey.md)). Per-user identity reaching a source is a per-source upgrade governed by **M16 (Identity Delegation & Source Trust)** with three levels — `service`, `asserted`, `delegated` — recorded per leg in the envelope (CC-13, ADR-0004 amendment 2026-09-09). M8 decides what the *answer* may contain; M16 decides which identity each leg runs under; M1 owns the service credential.
...
- **One pin table** — the compatibility matrix in the [architecture index](architecture/README.md) is the single source of truth for which version of each block the fabric currently builds against (RSA, `arangodb-schema-analyzer`, r2g, `arango-sparql-py` @ commit, AER, AOE, customer-context @ commit). *Reviewed against the tree 2026-09-15; the table now carries every block named here.*
...
### 10.13 Access control & identity (CC-13, M8 + M16 / P3–P4)
The user asking a question must be entitled to the data each leg returns; a query may be **partly answerable** (some legs permitted, others withheld). Today the engine connects to every source as one read-only service identity (CC-7) and `POST /federate` is **anonymous** — no per-user authorization — and the M5 single-leg **FILTER/OPTIONAL pushdown** (E1) runs under the *service* role, so source row-level security is **not** per-user and projected columns are unmasked. This CC commits the phased design; the full research, citations, and open decisions live in [access-control-research.md](architecture/access-control-research.md). It **extends CC-7** and realizes **M8 (Governance/OBAC)** + product-PRD §4.1 ("the steward and the asker"). *Revised 2026-09-09* after the vendor survey ([research/vendor-auth-access-control-survey.md](research/vendor-auth-access-control-survey.md)): the phases below are re-sequenced so that the shipped P3 baseline is named, an `asserted` level is inserted before full delegation, and brokers plus operator provisioning get an owner (**M16**).

- **Phase A0 — service identity + fabric-side policy (shipped, P3).** OIDC at the edge, one read-only service identity per source, M8's catalog policy subset enforced by the fabric (masking, row scope, governed seeds, citation redaction), fail-closed `delegated` seams with no broker. This is the market baseline; it stays certified and supported indefinitely, with the per-source read-only role published (M16 FR-1).
- **Phase A1 — asserted identity (P3.7).** The fabric still connects as itself but states the authenticated principal through a channel the source's own policies read — a Snowflake session variable consumed by row access policies, Postgres `SET ROLE` — so source-native RLS fires per user without an IdP federation project. The envelope labels the leg `asserted`; anonymous principals are refused. This is the pattern PuppyGraph ships for Snowflake, made honest about its audit meaning (the source records the service identity, not the user).
- **Ontop and the Postgres leg (decided 2026-09-10, after PJ's review of the M16 spec).** Ontop stays the Postgres rewriter *and* executor at every trust level; per-user identity on Postgres is delivered by changes **in Ontop**, not by replacing it. Ontop already carries HTTP headers into its `QueryContext` and, with `ontop.authorization=true`, exposes `ontop_user()` / `ontop_contains_role_or_group()` to mappings and lenses (Ontop PR #753); its maintainer has said a general impersonation solution should extend `QueryContext` and reuse connections per user (Ontop discussion #884). The options, in the order M16 pursues them: **(1) `asserted` via a statement-initializer change** — Ontop's per-statement `JDBCStatementInitializer` (Postgres subclass already sets fetch size) issues `SET ROLE` from `QueryContext.getUsername()` on borrow and `RESET ROLE` on close, so Postgres RLS fires per user on the shared pool; small, upstream-able, days of work. **(2) `delegated` via an identity-keyed `JDBCConnectionPool`** opening connections as the user (password passthrough, PG18 `oauth` bearer, or delegated Kerberos) — the design #884 is waiting for; pursue with Ontopic, not alone. **(3) Lenses-only filtering** with `ontop_user()` — zero code, but duplicates policy into lenses (M8's replication concern). **(4) Ontop reformulates, CDF executes** — demoted: Ontop's own maintainer notes the generated SQL needs a post-processing step to match SPARQL results exactly, and `/ontop/reformulate` is dev-mode only. **(5) Native Postgres executor** — fallback only; it would lose the aggregation pushdown that ADR-0005 grants solely to Ontop and Arango. Until (1) merges upstream, CDF carries a fork build of the Ontop image under CC-9 pin discipline. **ADR-0001 amended accordingly** (2026-09-10); M16 FR-5 holds the requirement.
- **Phase A2 — delegated identity.** Authenticate the user at the edge (OIDC) and **propagate identity to each source** (OAuth 2.0 Token Exchange / RFC 8693; DB `SET ROLE`; a per-source service-account-vs-passthrough choice, as data-virtualization products like Denodo expose) so source-native RLS/masking enforces for the *real* user. This is the highest-leverage step: it makes the E1 predicate pushdown **leak-safe** (masking/RLS fires *before* the pushed predicate) and removes the **confused-deputy** exposure of a broad service account. The NL/LLM front-end acts strictly with the user's authority (OWASP LLM06, "excessive agency"), never a shared identity.
...
**Cross-cutting invariants:** the user's identity reaches enforcement (no service-account-only path) at the highest level the source is certified for, and the level is recorded per leg; no level ever degrades silently to a lower one; a delegated token that expires before the leg deadline is refused at admission, never refreshed mid-leg; leg and assembly caches are keyed on entitlement scope; citations are governed like data; scope/filter over hard-deny to avoid existence leaks; source-native controls stay in force beneath any ontology policy; the ArangoDB hub, which has no document-level security, is fabric-enforced at every level. **Ownership:** M1 holds service credentials, **M16** holds trust levels, brokers, provisioning contracts and certification goldens, M8 holds decisions, M5 carries the context. **Decision record:** **ADR-0004** — identity planes & OBAC enforcement points (leg vs reassembly vs both; ReBAC vs ABAC; tenancy; citation redaction; refusal classes).
