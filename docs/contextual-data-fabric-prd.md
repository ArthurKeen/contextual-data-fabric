...
> **Status:** Draft v0.1 for team review. Posted per the [[2026-07-13 the design partner Customer Context Roadmap]] action item ("PJ to draft PRD"). Arthur needs this before refactoring r2g / the ontology extractor so we scope the build rather than over-build.
>
...
| **Phase 2** | Highest-value connector + assembled pattern | **Snowflake** connector (pending free-tier check); the **assembled/materialized** query pattern for analytics; richer ontology **alignment** across ≥2 structured sources + unstructured; cost/latency instrumentation to directly answer the data-platform lead's token objection. |
...
- (Nice-to-have) A note/measurement on **cost/latency** vs a naive approach, to pre-empt the data-platform lead's objection.
