...
> **Status 2026-09-15.** r2g is **0.4.1** (requires `relational-schema-analyzer>=0.8.0,<0.9.0`; RSA is 0.8.0). **RE-1 is delivered**: `export-csi` / `export-r2rml` are what produced every relational CSI under `deploy/csi/`, and `export-csi` forwards RSA 0.8.0's bitemporal stamps; r2g loads the CSI schema from `arangodb-schema-analyzer` with its vendored copy as fallback. **RE-2 is resolved** by Ontop's adoption (ADR-0001 #2, amended 2026-09-10). The "no federated path" observation below is historical.
