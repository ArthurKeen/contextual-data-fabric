...
>
> **Status 2026-09-15 — RE-1 superseded; not on CDF's structured path.** A code-read on 2026-09-14 (PR #24, M2 §6) showed CDF's structured→ontology path is **r2g → RSA → CSI v1 + R2RML**, with the Arango hub's CSI from `arangodb-schema-analyzer`; nothing in Phase 1 gates on AOE. AOE (1.9.0; RSA `>=0.8.0,<0.9.0`, `mcp>=1.0,<2`) now **imports CSI v1 as ontologies** (`app/services/csi_import.py`; `POST /api/v1/ontology/schema/csi/preview` and `/import`; MCP tools) — that, not an RSA-bundle handoff, is its structured entry point, and it sits between the analyzers and CDF as the curation step in the target design. The ownership quote in §1 describes AOE's own product scope. RE-2 (alignment) and RE-3 (belief/time-travel) stand.
...
- **RE-1 (P1) ~~★~~ ~~RESOLVED → wiring~~ SUPERSEDED (2026-09-15):** structured→ontology **exists** in AOE (see §1) but CDF does not call it — its relational CSI/R2RML come from r2g via RSA. ~~Remaining work: wire the RSA metadata bundle → AOE SQL→OWL/SHACL mapping handoff~~ Replaced by AOE's CSI v1 import (shipped 2026-09-14) and, still open, retiring AOE's own LPG type detector in favour of ASA's answer carried in CSI.
...
- ~~Does AOE already do structured→ontology, or is that r2g's job?~~ **Answered (v0.2):** AOE owns the SQL→OWL/SHACL mapping; RSA is the read-only physical-schema introspector; r2g is the reference application composing RSA (its Phase 10 adds LLM-assisted derivation + review UI). The split is documented in AOE's README. **Corrected (2026-09-15):** for CDF the answer is r2g's — every relational CSI the fabric reads is r2g-emitted through RSA; AOE's derivation is its own and is not consumed.
