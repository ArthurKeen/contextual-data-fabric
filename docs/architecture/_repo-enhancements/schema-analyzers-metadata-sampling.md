...
> **Status 2026-09-15.** RSA is **0.8.0** (bitemporal stamping, `bitemporal.py`). `arangodb-schema-analyzer` is **0.14.0**: 0.13.0 added bitemporal stamping (`transactionTime`, `validTime.from`, `validTimeSource`, `predecessorFingerprint`), 0.13.1 widened the CSI v1 `validTimeSource` enum to five values (`catalog`, `event`, `file`, `fingerprint-continuity`, `observed`), and 0.14.0 converged **LPG type detection, which ASA now owns for the portfolio**. The version numbers below are as of v0.2.
