...
- **B1** — Ontop relational leg (Apache-2.0; the adopt-vs-build fork resolved to
...
- **IR = SPARQL** (canonical), generated by **reusing `arango-cypher-py`'s NL
  ...
  adopt — Apache-2.0 OSS, free); Arango leg = **`arango-sparql-py`** (SPARQL→AQL, owned, finish it).
...
| r2g (mapping + pushdown) | mapping export exists; Phase 12 planned | **forward-CSI + R2RML emitter** (P12.1, shipped); pushdown (P12.2) retired — relational leg adopts Ontop (Apache-2.0 OSS, free) |
