...
| **M14** | **Developer Surfaces** | The fabric's **consumable contracts**: catalog-manifest publications (ADR-0007 D6), ontology-map payload, answer envelope, `/federate`, gate status. The admin console (onboarding wizard, catalog browser, controller review queue) is **delegated to ArGOS** as fabric tabs (product PRD §5). Agents reach the fabric through MCP (PRD §10.2); no other API skin is planned (a GraphQL surface was considered and dropped 2026-09-17). | Contracts |
...
| M14 Developer Surfaces | `/federate` + answer envelope + `/nl-preview`; manifest as checked-in file + `cdf-catalog` CLI; MCP `list_sources` / `list_concepts` | Versioned read-only publication contract (ADR-0007 D6; S6, thin surface S3), gate-status and ontology-map endpoints |
