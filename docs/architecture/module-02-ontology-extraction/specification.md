...
## 6. Dependencies
- **Repos:** `r2g` (structured), `ontology-extractor`/AOE (unstructured). **Critical:** confirm/complete the structured→ontology path — see [[contextual-data-fabric/docs/architecture/_repo-enhancements/ontology-extractor-structured|enhancement]]. This is the make-or-break dependency from the [[the design partner Feedback Summary]].
