...
- **Produces:** a **mapping artifact** keyed by ontology element → per-source realization + transforms, consumed by M5 (and r2g's query generation). **Canonical form (ADR-0001 #3): `CSI v1`** — the cross-tool interchange defined in `arangodb-schema-analyzer` (`{csiVersion, conceptualModel, arangoPhysicalMapping, provenance{direction}, statistics}` — the top-level key is `arangoPhysicalMapping`, not `physicalMapping`; property-level physical mapping lives at `arangoPhysicalMapping.entities.<E>.properties.<p>.field`, as every document in `deploy/csi/` shows), with **r2g as the forward producer** (P12.1; shipping in r2g 0.4.1 `export-csi`) — plus two serializations: **CSI → R2RML** for the SQL/Ontop leg (r2g 0.4.1 `export-r2rml`) and **CSI → MappingBundle/OWL-Turtle** for the AQL transpiler (`arango-sparql-py`; CDF has no `arango-cypher-py` dependency). OSI/YAML remains the interop export (FR-6). ~~Known bug to absorb: the `phys:` namespace mismatch between `arango-sparql-py` and the analyzers (ADR-0001 #3.4).~~ **Closed 2026-07-15** (M5 plan WP A2, `arango-sparql-py` `64027b6`: the analyzer `phys:` namespace is accepted as canonical).
...
- **FR-7 (P1 minimal / P2 full):** **Mapping versioning** (PRD §10.3 / CC-3) — since the mapping *is* the query, an unversioned mapping is an unversioned query. P1: the artifact carries a version/hash that is cited in the answer envelope. P2: mapping versions align with ontology versions via the same temporal pattern AOE already uses. **Note (2026-09-15):** the producers now stamp CSI bitemporally — ASA 0.13.0 emits `provenance.transactionTime`, `validTime.from`, `validTimeSource`, `predecessorFingerprint`; RSA 0.8.0 does the same for relational bundles; r2g 0.4.1 `export-csi` forwards them. The open part is **CDF-side consumption**: the catalog does not yet read these fields into the version/hash cited in the envelope or use them for as-of mapping selection.
- **FR-8 (P1):** **Statistics pass-through** (PRD §10.11 / CC-11) — the analyzers already emit row/collection counts, FK cardinality hints, and per-field value distributions; the CSI v1 artifact and its serializations **preserve these statistics** so the M5 planner receives them (join ordering, admission control, bind-join sizing). Dropping statistics in translation re-blinds the planner. Sample *values* carried in the artifact respect the analyzers' redaction options (no Restricted/PII sample values into LLM prompts — CC-7 / r2g Phase 9 gates).
...
- ~~Mapping artifact representation~~ **Answered (v0.3) by ADR-0001 #3 code-read: `CSI v1` is the hub** (already designed in `arangodb-schema-analyzer` with r2g named as forward producer); the four adapters (r2g forward-CSI emitter, CSI→R2RML, CSI→MappingBundle, `phys:` namespace fix) are M5 plan WPs A1–A4. **Status (verified 2026-09-15 against the M5 implementation plan):** all four shipped — A1 r2g forward-CSI emitter and A4 CSI→R2RML serializer are the r2g 0.4.1 `export-csi` / `export-r2rml` commands (every relational CSI under `deploy/csi/` carries `provenance.producer: r2g`); A2 `phys:` acceptance (2026-07-15, `arango-sparql-py` `64027b6`) and A3 CSI→`MappingBundle` adapter (2026-07-15, `a1ef785`, landed in `arango-sparql-py` rather than r2g). Only the OSI/YAML wrap question above remains.
