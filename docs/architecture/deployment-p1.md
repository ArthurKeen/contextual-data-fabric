...
3. **The UI talks only to the M5 engine; the engine talks to the sources.** Credentials live in the engine's env (`.env`, read-only Postgres role — CC-7); the browser never sees a connection string. The demo role also carries **`statement_timeout` + a memory cap** (CC-11 defense in depth — the source protects itself even if the engine misbehaves), and the engine ships with per-leg LIMITs/timeouts from day one.
