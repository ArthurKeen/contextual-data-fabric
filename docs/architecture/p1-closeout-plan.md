...
> **Update 2026-08-04 — Q12 dropped.** The former "green metrics / red sentiment"
> centerpiece (Q12) is deprecated — the sentiment/entity extraction it needed is
> cut. The arc is now the **5-question** order **Q7 → Q2 → Q9 → Q5 → Q8**, the
> gated slice is **Q7 → Q2**, and **Q2** (renewal risk + WHY) is the lead
> cross-source question. Every Q12-specific reference below — the six-question
> arc, the Q7→Q2→Q12 gate slice, the cut-lines, the `Q12★` golden regex, and
> P1.3's Q12 extraction — is **superseded by this note**.
...
| P1.3 unstructured→Arango | ✅ **structural v0** — `deploy/arango/load_corpus.py` (80 docs/80 chunks, `account_id` stamp 100%) + `export_csi.py` (analyzer reverse CSI). ~~**Remaining for customer-grade:** PJ's LLM-extraction pipeline for the Q12 sentiment/entity content~~ — **deprecated 2026-08-04 (Q12 dropped).** |
...
| P1.6 golden gate | ✅ contracts encoded (reconciliation / anchor / citations-empty / partial-failure); **15 live cases green** — g1–g7 plus the **locked 5-question arc as prepared prompts + goldens** (g08 Q7 anchor across two structured systems, g09 Q2 lead cross-source, g10 Q9, g11 Q5, g12 Q8) and the **adversarial set** (g13 ghost-account grounded-EMPTY, g14 PII refuse-zero-citations, g15 injection-is-data). Completed 2026-08-04, post-Q12-drop |
...
P1 gates on the **Q7 → Q2 slice + 2 refusal cases**; the rest of the arc fills in as data lands.
...
A top-level `make demo` (or `deploy/docker-compose.yml`): both stacks + corpus loads + the service + the browser demo, ports parameterized, fresh-clone-to-answer in one command. Converge `deploy/demo/federation.py` onto `FederationService.from_env` (one wiring path, not two). **The demo script is the locked arc in its locked order** — Q7 (anchor: clean structured-only sourcing builds trust) → Q2 → Q9 → Q5 → Q8 (Q12★ dropped 2026-08-04) — followed by an adversarial refusal, plus the fabric's own act: kill Postgres mid-demo → the declared-partial envelope. Per PJ's own runbook rule: **the golden gate runs before any demo, no exceptions.**
...
**Cut lines if time runs out (in order):** drop P1.5's free-form act (registry runs the locked prompts — the script doesn't change); trim the arc to its gated slice **Q7 → Q2** (the anchor + the risk question carry the story); **never cut** P1.6's refusal/partial cases or P1.7's pre-demo gate run — a flaky demo is the named failure mode (M9 NFR), and "run the eval gate before any demo" is PJ's own runbook rule.
