...
- **Recover the 5+2 missing question texts:** PJ to commit `locked-questions-and-data-map.md` and `locked-questions-expected-answers.md` to `customer-context/docs/research/` (they're cited by committed docs but absent — the `.planning` verification proves they existed with per-question data maps and dual-graph proofs).
